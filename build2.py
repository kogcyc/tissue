...
STATIC_DIR = "static"            
PARTIALS_DIR = "partials"        # NEW

# Default Templates
INDEX_TEMPLATE = "index.html"  
DEFAULT_TEMPLATE = "default.html"

# Ensure build and template directories exist
os.makedirs(BUILD_DIR, exist_ok=True)
os.makedirs(TEMPLATES_DIR, exist_ok=True)

# --- NEW: Render Markdown partials into _filename.html in templates ---
for filename in os.listdir(PARTIALS_DIR):
    if filename.endswith(".md"):
        input_path = os.path.join(PARTIALS_DIR, filename)
        name_without_ext = os.path.splitext(filename)[0]
        output_filename = f"_{name_without_ext}.html"
        ...
        print(f"Rendered partial: {input_path} → {output_path}")
